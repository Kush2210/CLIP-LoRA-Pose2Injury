...
def build_clip_model(backbone: str, lora_ckpt: Path, use_lora: bool, dataset: str = "tromnet", shots: int = 16,
...
def summarize_probs(probs_clip: np.ndarray, probs_lora: np.ndarray):
...
def make_image_report(
    img_path: Path,
    pose_model,
    clip_model_clip_only,
    preprocess_clip_only,
    text_features_clip_only,
    clip_model_lora,
    preprocess_lora,
    text_features_lora,
    device: str,
    ...
    for ax, part_name in zip(axes[2:], part_titles):
        ...
        probs_clip = predict_probs(mask_path, clip_model_clip_only, preprocess_clip_only, text_features_clip_only, device)
        probs_lora = predict_probs(mask_path, clip_model_lora, preprocess_lora, text_features_lora, device)
        pred = summarize_probs(probs_clip, probs_lora)
        ...
        ax.set_title(
            ...
            f"combined_injury={pred['injury_score']:.4f}, threshold={INJURY_SCORE_THRESHOLD:.2f}\n"
            f"clip_injury={pred['clip_injury_score']:.4f}, lora_injury={pred['lora_injury_score']:.4f}",
            fontsize=10,
            color=color,
        )
        ax.axis("off")
...
def main():
    ...
    parser.add_argument("--input_dir", default=str(PROJECT_ROOT / "testing"))
    ...
    device_clip, clip_model_clip_only, preprocess_clip_only, runtime_clip = build_clip_model(
    ...
    if device_clip != device_lora:
        raise RuntimeError(f"Model device mismatch: clip={device_clip}, lora={device_lora}")

    text_features_clip_only = build_text_features(clip_model_clip_only, device_clip, CLASS_NAMES, PROMPT_TEMPLATES)
    text_features_lora = build_text_features(clip_model_lora, device_lora, CLASS_NAMES, PROMPT_TEMPLATES)

    print(
        "Runtime (clip-only):",
        f"torch={runtime_clip['torch_version']},",
        f"sdp={runtime_clip['has_sdp']},",
        f"device={runtime_clip['device']}",
    )
    print(
        "Runtime (clip+lora):",
    ...
    for img_path in image_paths:
        result = make_image_report(
            ...
            clip_model_clip_only=clip_model_clip_only,
            preprocess_clip_only=preprocess_clip_only,
            text_features_clip_only=text_features_clip_only,
                ...
                writer.writerow(
                    {
                        "image": row["image"],
                        "part": part["part"],
                        "predicted_class": part["predicted_class"],
