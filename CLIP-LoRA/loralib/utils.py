...
def load_lora(args, list_lora_layers):
    # Allow either a direct checkpoint file or the original directory layout.
    if args.save_path.endswith('.pt') and os.path.isfile(args.save_path):
    ...
    loaded_data = torch.load(load_path, map_location=map_location)
